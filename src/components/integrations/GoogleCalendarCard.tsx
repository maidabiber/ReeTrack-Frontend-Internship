  ...
  return (
    <>
      ...
      <div className="rounded-[18px] bg-white p-5 shadow-card">
        <div className="flex flex-wrap items-start gap-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            {isConnected && connection.providerAccountId && (
              <p className="mt-2 font-mono text-[12px] text-navy/55">{connection.providerAccountId}</p>
            )}
            ...
            {isConnected && syncStatus === CalendarSyncStatus.Error && connection.lastSyncError && (
              <p className="mt-2 text-[12.5px] leading-[1.5] text-red">{connection.lastSyncError}</p>
            )}

            {actionError && (
              <p className="mt-2 text-[12.5px] leading-[1.5] text-red">{actionError}</p>
            )}
          </div>
          ...
          <div className="flex flex-shrink-0 flex-col items-end gap-3">
            ...
            {!isLoading && loadError && (
              <button
                ...
                className="rounded-full border-[1.5px] border-navy px-3.5 py-1.5 font-display text-[12px] font-semibold text-navy"
              >
                Retry
              </button>
            )}
        ...
        {!isLoading && loadError && (
          <p className="mt-4 text-[13px] text-red">{loadError}</p>
        )}

        {!isLoading && !loadError && !isConnected && (
          <div className="mt-4 flex justify-end">
            <GoogleCalendarConnectButton className="flex cursor-pointer items-center gap-2 rounded-full border-2 border-navy bg-white px-4 py-2 font-display text-[13px] font-semibold text-navy transition-colors hover:bg-cream-card">
        ...
        {!isLoading && !loadError && isConnected && (
          <div className="mt-4 flex flex-wrap justify-end gap-2">
            <button
              ...
              className="rounded-full border-[1.5px] border-navy bg-transparent px-4 py-2 font-display text-[13px] font-semibold text-navy disabled:cursor-not-allowed disabled:opacity-60"
            >
              Disconnect
            </button>
            <button
              ...
              className="rounded-full bg-brand px-4 py-2 font-display text-[13px] font-semibold text-white transition-colors hover:bg-brand-deep disabled:cursor-not-allowed disabled:opacity-60"
            >
        ...
        >
          <div className="mt-[18px] flex gap-2">
            <button
              ...
              className="flex-1 rounded-full border-[1.5px] border-navy bg-transparent py-2.5 font-display text-[13px] font-semibold text-navy disabled:cursor-not-allowed disabled:opacity-60"
            >
              Cancel
            </button>
            <button
