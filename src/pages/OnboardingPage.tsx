...
function WelcomeStep({ onContinue }: { onContinue: () => void }) {
  return (
    <>
      ...
      <Blob className="h-[30px] w-[30px] top-[200px] left-[400px] -rotate-6 rounded-sm bg-brand-tint" />
      ...
      <div className="relative z-[2] flex flex-col items-center px-16 text-center">
        ...
        <h1 className="mt-7 mb-4.5 max-w-[680px] font-display text-3xl leading-[1.08] font-bold">
          Welcome.
        </h1>
        <p className="mb-14 max-w-auth text-xl leading-[1.6] text-navy/70">
        ...
        <span className="mt-4.5 font-display text-body font-semibold text-navy/55">Click to continue</span>
      </div>
    </>
  )
}
  ...
  return (
    <>
      ...
      <div className="relative z-[2] flex flex-col items-center px-16">
        <div className="w-auth max-w-full rounded-4xl bg-brand-gradient p-px shadow-auth">
          <div className="flex flex-col items-center rounded-4xl bg-white px-14 py-[52px] text-center">
            <LogoMark className="mb-5 h-9" />
            <h1 className="mb-3 font-display text-2xl leading-[1.25] font-bold">
              Create the first administrator account
            </h1>
            <p className="mb-8 text-body-lg leading-[1.6] text-navy/70">
...
function StepBadge({ label }: { label: string }) {
  return (
    <div className="absolute top-0 right-0 left-0 z-[3] flex items-center justify-end px-14 py-9">
      <span className="rounded-full bg-brand-tint px-4 py-2 font-mono text-sm font-medium tracking-[0.12em] text-brand uppercase">
        {label}
      </span>
    </div>
  )
